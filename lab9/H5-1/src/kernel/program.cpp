...
bool ProgramManager::copyProcess(PCB *parent, PCB *child)
{
    ...
    child->parentCount = parent->count;
    ...
    if (program->pageDirectoryAddress)
    {
        ...
        for (int i = 0; i < 768; ++i)
        {
            ...
            for (int j = 0; j < 1024; ++j)
            {
                ...
                memoryManager.releasePhysicalPages(1, paddr, 1);
            }
        ...
        memoryManager.releasePages(0, (int)pageDir, 1);
        
        int bitmapBytes = ceil(program->userVirtual.getlen() / 8.0);
        int bitmapPages = ceil(bitmapBytes / 4096.0);

        memoryManager.releasePages(0, (int)program->userVirtual.getBitMap(), bitmapPages);

        PCB * search;
        ListItem *item= this->allPrograms.head.next;
        while (item)
        {   
            search= (PCB *)((int)item-(int)&(((PCB *)0)->tagInAllList));
            if(search->parentPid == program->pid)
            {   if (search->status == 4)
                {   
                    myprintf("Zombie Process(pid:%d) recycled\n",search->pid);
                    releasePCB(search);
                }
                else
                {
                    search->parentPid = program->parentPid;
                    search->parentCount = program->parentCount;
                    myprintf("Process %d is process %d's child now\n",search->pid,program->parentPid);
                }
            }
            item = item->next;
        }
...
int ProgramManager::wait(int *retval)
{
    ...
    while (true)
    {
        ...
        while (item)
        {   
            child = (PCB *)((int)item-(int)&(((PCB *)0)->tagInAllList));
